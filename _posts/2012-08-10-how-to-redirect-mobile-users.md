---
layout: post
title:  "How To Redirect Mobile Users"
...
ASP:
	...
	<%
	   ...
	   b.Global=true
	   v.Global=true
	   if b.test(u) or v.test(Left(u,4)) then response.redirect("http://yourwebsite.com/mobile/") end if
	%>
	


Just replace http://yourwebsite.com/mobile/ with the url for your mobile website.  

If you want to do the same for tablets just replace `android.+mobile` and `ip(hone|od)` with **`android`** and **`ip(ad|hone|od)`**